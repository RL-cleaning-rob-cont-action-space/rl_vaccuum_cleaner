...
class ContinuousVacuumCleanerEnv(gym.Env):
    ...
    def __init__(self, size=10.0, resolution=50, coverage_radius=1.0, max_steps=2000):
        super().__init__()
        self.size = size
    ...
    def step(self, action):
        ...
        self.agent_orientation = (
            (self.agent_orientation + ang_vel * self.dt + np.pi) % (2 * np.pi)
        ) - np.pi
        ...
        return (
            ...
            {"coverage_percentage": self.coverage_percentage, "steps": self.steps},
        )
    ...
    def _calculate_reward(self, newly_covered):
        ...
        recent_coverage = np.mean(
            self.coverage_grid[
                max(0, int((self.agent_position[1] - 1) / self.cell_size)) : min(
                    self.resolution, int((self.agent_position[1] + 1) / self.cell_size)
                ),
                max(0, int((self.agent_position[0] - 1) / self.cell_size)) : min(
                    self.resolution, int((self.agent_position[0] + 1) / self.cell_size)
                ),
            ]
        )
        return (
    ...
    def render(self, mode="human"):
        ...
        end_x = self.agent_position[0] + self.coverage_radius * 1.5 * math.cos(
            self.agent_orientation
        )
        end_y = self.agent_position[1] + self.coverage_radius * 1.5 * math.sin(
            self.agent_orientation
        )
        self.ax.plot(
