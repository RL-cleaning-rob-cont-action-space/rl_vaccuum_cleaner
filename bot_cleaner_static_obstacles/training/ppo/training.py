...
from bot_cleaner_static_obstacles.algos.ppo import PPO
import torch
import numpy as np
import time
    ...
    try:
        for episode in range(1, max_episodes + 1):
            ...
            if coverage > best_coverage:
                ...
                torch.save(agent.policy.state_dict(), f"bot_cleaner_static_obstacles/models/ppo/best_model_{best_coverage:.2f}.pth")
                print(f"New best model saved with coverage: {best_coverage:.2%}")
            
            # Periodic model saving
            if episode % save_interval == 0:
                torch.save(agent.policy.state_dict(), f"bot_cleaner_static_obstacles/models/ppo/model_ep_{episode}.pth")
        ...
        torch.save(agent.policy.state_dict(), "bot_cleaner_static_obstacles/models/ppo/final_model.pth")
        env.close()
