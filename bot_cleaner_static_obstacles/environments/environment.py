...
class GridMazeVacuumCleanerEnv(gym.Env):
    ...
    def __init__(self, size_x=10, size_y=10, coverage_radius=1.0, max_steps=1000):
        super().__init__()
        self.size_x = size_x
        self.size_y = size_y
    ...
    def _define_maze_layout(self):
        ...
        wall_positions = [3, 7]
        gap_start = self.size_y // 2 - 2  # Adjust gap for smaller grid
        gap_height = 4
...
def main():
    ...
    for _ in range(1000):  # Run for 1000 steps max
        ...
        if done:
            print(
                f"Episode finished! Coverage: {info['coverage_percentage']:.2%}, Steps: {info['steps']}"
            )
            break
