...
# Add both parent directories to path for imports
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train a robot cleaner agent using SAC")
...
def plot_learning_curve(steps, scores, avg_scores, filename):
    ...
    ax.set_ylabel('Score')
    ax.set_title('SAC Learning Curve')
    ax.grid(True)
...
def train_sac():
    ...
    try:
        ...
        print(f"Starting SAC training for {args.total_steps} steps...")
        ...
        while total_steps < args.total_steps:
            ...
            if total_steps % args.eval_interval == 0:
                ...
                if eval_stats['mean_reward'] > best_eval_reward:
                    ...
                    model_path = os.path.join(args.model_dir, 'sac_best_model.pt')
            ...
            if total_steps % args.save_interval == 0:
                ...
                model_path = os.path.join(args.model_dir, f'sac_checkpoint_{total_steps}_{timestamp}.pt')
                agent.save(model_path)
        ...
        # Save final model
        model_path = os.path.join(args.model_dir, 'sac_final_model.pt')
        agent.save(model_path)
