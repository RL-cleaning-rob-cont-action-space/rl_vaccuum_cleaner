...
class EnhancedActorCritic(nn.Module):
    def __init__(self, grid_size_x=10, grid_size_y=10):
        ...
        # Process dirt grid (new addition)
        ...
        # Shared network (expanded to include dirt features)
        self.shared_net = nn.Sequential(
            nn.Linear(256 + 256 + 256 + 128, 512),  # Added 256 for dirt features
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.ReLU(),
        )
    ...
    def forward(self, coverage, walls, dirt, position):
        ...
        # Process dirt grid
        dirt_features = self.dirt_net(dirt)
...
class EnhancedPPO:
    def __init__(self, env):
        ...
        self.policy = EnhancedActorCritic(
    ...
    def act(self, obs):
        ...
        dirt = torch.FloatTensor(obs["dirt"]).view(1, self.env.size_y, self.env.size_x)
        position = torch.FloatTensor(obs["position"]).view(1, -1)

        with torch.no_grad():
            coverage = coverage.to(self.device)
            walls = walls.to(self.device)
            dirt = dirt.to(self.device)
            position = position.to(self.device)

            mean, std, value = self.policy(coverage, walls, dirt, position)
    ...
    def update(self, rollout):
        ...
        coverages = torch.FloatTensor(np.array([t["coverage"] for t in rollout])).view(
            -1, self.env.size_y, self.env.size_x
        )
        walls = torch.FloatTensor(np.array([t["walls"] for t in rollout])).view(
            -1, self.env.size_y, self.env.size_x
        )
        dirt = torch.FloatTensor(np.array([t["dirt"] for t in rollout])).view(
            -1, self.env.size_y, self.env.size_x
        )
        ...
        for _ in range(self.epochs):
            ...
            for start in range(0, len(indices), self.batch_size):
                ...
                mean, std, values = self.policy(
                    cov_batch, wall_batch, dirt_batch, pos_batch
                )
                dist = Normal(mean, std)
                ...
                surr2 = (
                    torch.clamp(ratios, 1 - self.epsilon_clip, 1 + self.epsilon_clip)
                    * adv_batch
                )
